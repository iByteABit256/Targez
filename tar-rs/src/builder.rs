...
impl<W: Write> Builder<W> {
    ...
    /// ```
    pub fn append_path<P: AsRef<Path>>(&mut self, path: P, exclude_vcs: bool) -> io::Result<()> {
        let mode = self.mode.clone();
        let follow = self.follow;
        append_path_with_name(self.get_mut(), path.as_ref(), None, mode, follow, exclude_vcs)
    }
    ...
    /// ```
    pub fn append_dir_all<P, Q>(&mut self, path: P, src_path: Q, exclude_vcs: bool) -> io::Result<()>
    where
        P: AsRef<Path>,
        Q: AsRef<Path>,
    {
...
fn append_path_with_name(
    ...
    if stat.is_file() && (!exclude_vcs || !is_file_vcs_related(path)) {
...
fn append_dir_all(
    ...
    while let Some((src, is_dir, is_symlink)) = stack.pop() {
        ...
        if is_dir || (is_symlink && follow && src.is_dir()) {
            if  exclude_vcs && is_file_vcs_related(&dest) {
                continue;
            }
...
fn is_file_vcs_related(path: &Path) -> bool {
	if let Some(file_name) = path.file_name() {
		if path.is_dir() {
            if ".git".eq(file_name) ||
                ".svn".eq(file_name) ||
                "CVS".eq(file_name) ||
                ".DS_Store".eq(file_name) ||
                "TEST321312".eq(file_name) ||
                "Thumbs.db".eq(file_name) {
			    return true;
            }
		}
	}
